...
registerInfo({
  ...
  bg: { t:'Background priority',
    d:'Leaves audio completely unrouted so it keeps playing when your screen locks. DSP <i>and</i> the visualisers are unavailable in this mode, because both need the audio engine attached.',
    w:'On iOS especially, an audio engine can be suspended when the screen locks — an OS restriction, not something a web page can work around. Leave this off for the full experience; turn it on when you want guaranteed lock-screen playback.' },
  ambient: { t:'Ambient spectrum',
    d:'The soft curve along the bottom of the page, drawn from the audio and tinted from the album art. Purely decorative.',
    w:'Needs the audio engine, so it won\'t draw while Background priority is on.' },
  dust: { t:'Floating particles',
    d:'Slow drifting motes that rise faster and glow brighter with bass energy. They keep drifting even when nothing is playing.' },
...
/* ============================================================
   ...
   The visualisers need an analyser, which needs Web Audio. Rather
   than force the whole processing chain on, this builds the
   minimum: source -> analyser -> output. Sonically transparent,
   but it does route audio through Web Audio, which is why
   Background priority disables it.
   ============================================================ */
DSP.monitorOnly = false;

DSP.buildMonitor = async function () {
  if (this.built || this.monitorOnly) return true;
  try {
    const AC = window.AudioContext || window.webkitAudioContext;
    if (!AC) return false;
    this.ctx = new AC({ latencyHint: 'playback' });
    this.src = this.ctx.createMediaElementSource(AURA.audio);
    ...
    console.warn('[Aura] monitor graph failed:', e);
    return false;
  }
};

/* Upgrade from monitor to the full chain without interrupting audio:
   the MediaElementSource is reused, only what's after it changes. */
const _origBuild = DSP.build.bind(DSP);
DSP.build = async function () {
  if (this.built) return true;

  if (this.monitorOnly && this.ctx && this.src) {
    try {
      const c = this.ctx, n = this.nodes;
      try { this.src.disconnect(); } catch (e) {}
      try { n.an.disconnect(); } catch (e) {}
      ...
      n.sp = c.createChannelSplitter(2); n.mg = c.createChannelMerger(2);
      n.mid = c.createGain(); n.side = c.createGain();
      n.l2m = c.createGain(); n.r2m = c.createGain();
      n.l2s = c.createGain(); n.r2s = c.createGain();
      n.m2l = c.createGain(); n.m2r = c.createGain();
      ...
      n.m2l.gain.value = 1; n.m2r.gain.value = 1;
      n.s2l.gain.value = 1; n.s2r.gain.value = -1;
      n.xsp = c.createChannelSplitter(2); n.xmg = c.createChannelMerger(2);
      n.dl = c.createGain(); n.dr = c.createGain();
      n.xdl = c.createDelay(.01); n.xdr = c.createDelay(.01);
      n.xdl.delayTime.value = .00027; n.xdr.delayTime.value = .00027;
      n.xll = c.createBiquadFilter(); n.xlr = c.createBiquadFilter();
      ...
      console.error('[Aura] upgrade failed:', err);
      toast('Could not start the full chain: ' + err.message, 'err');
      return false;
    }
  }
  return _origBuild();
